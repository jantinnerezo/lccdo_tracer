<?php
    ...
    if(isset($_GET['verify'])){
        ...
        $graduate_id = $_GET['verify'];

        try{
            // Sql query
            $sql = 'UPDATE tbl_graduates SET status = 1 WHERE graduate_id = :graduate_id';
    ...
    function job_history($pdo,$graduate_id){
     ...
     // Delete a graduate
     if(isset($_POST['delete_graduate'])){
        ...
        try{
            // Sql query
            $sql = 'DELETE FROM tbl_graduates WHERE graduate_id = :graduate_id';

            $stmt    = $pdo->prepare($sql); // Prepared statement
            $stmt->bindParam(':graduate_id', $graduate_id, PDO::PARAM_STR);
            $stmt->execute();
            delete_jobs($pdo, $graduate_id);
    ...
    // Delete also the jobs
    function delete_jobs($pdo, $graduate_id){

        try{
            // Sql query
            $sql = 'DELETE FROM tbl_job_history WHERE graduate_id = :graduate_id';
